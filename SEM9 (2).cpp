...
public:
	...
	Tir(const char* tara, float tonaj ,int nrRoti,string marca,string* producatori ,float pret,int an) :Masina(nrRoti,marca,pret,an) {
		this->tara = new char[strlen(tara) + 1];
		strcpy_s(this->tara, strlen(tara) + 1, tara);
		this->tonaj = tonaj;
	...
	//int _nrRoti, string marca, float pret, int an) :anFabricatie(an)
