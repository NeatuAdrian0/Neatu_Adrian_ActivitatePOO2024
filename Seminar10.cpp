...
public:
	...
	void scriereFiserBinar(fstream &fisier) {
		/*int nrRoti;
		string* producatori;
		string marca;
		float pret;
		...
		for (int i = 0; i < nrRoti; i++) 
		{
			...
			fisier.write((char*)(producatori[i].c_str()), lungime * sizeof(char)); // char* in sir de caractere
		}
	...
	void citireFiserBinar(fstream& fisier) {
		/*int nrRoti;
		string* producatori;
		string marca;
		float pret;
		...
		for (int i = 0; i < nrRoti; i++)
		{
			...
			fisier.read((char*)(producatori[i].c_str()), lungime * sizeof(char)); // char* in sir de caractere
		}
		int lungime2 = 0;
...
ofstream& operator<<(ofstream& output, Masina m) {
	output << m.marca << endl;
	output << m.nrRoti << ": ";
	if (m.producatori != NULL) {
		for (int i = 0; i < m.nrRoti; i++) {
...
int main() {
	...
	//m.scriereFiserBinar(fisierBinar);
	//fisierBinar.close();

	//in citeste
	fstream fisierBinar1("fisierBinar1.txt", ios::binary | ios::in);
	Masina m1;
	m1.citireFiserBinar(fisierBinar1);
	cout << m1;
